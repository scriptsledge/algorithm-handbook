...
-   :material-youtube: **Watch the Solution**
    [Watch Video](https://youtube.com/shorts/TRbzjm3yej0){ .md-button }

-   :material-github: **View on GitHub**
    [Source Code](https://github.com/scriptsledge/algorithm-handbook/tree/main/src/2874){ .md-button }

-   :simple-leetcode: **LeetCode Solution**
    [Read Post](https://leetcode.com/problems/maximum-value-of-an-ordered-triplet-ii/solutions/7458925/c-on-solution-prefix-suffix-arrays-clean-1kq6){ .md-button }
...
We can use **Prefix Max** and **Suffix Max** arrays to store these maximum values.
...
=== "C++"
    ...
    ```cpp
    --8<-- "src/2874/solution.cpp"
    ```

## 📊 Complexity Analysis

*   **Time Complexity:** $O(N)$
    *   We traverse the array to build `prefixMax` ($O(N)$).
    *   We traverse the array to build `suffixMax` ($O(N)$).
    *   We traverse the array to find the maximum triplet value ($O(N)$).
    *   Total time complexity is dominated by these linear passes.

*   **Space Complexity:** $O(N)$
    *   We use two additional arrays `prefixMax` and `suffixMax` of size $N$.