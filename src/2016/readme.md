...
We iterate through the array once while keeping track of the minimum element seen so far (`minSoFar`). For each element `nums[i]`, if it is greater than `minSoFar`, we calculate the difference `nums[i] - minSoFar` and update our `maxDiff`. If `nums[i]` is smaller than `minSoFar`, we update `minSoFar` to `nums[i]`. This ensures we always subtract the smallest possible value found before the current index.
